	...
	while (childs != ctx->children.end()) {
		...
		if (binOp) {
			while (true) {
				ch2 = *childs;
				childs++;
				auto aic = (Verilog2001Parser::Attribute_instanceContext*) (ch2);
				if (aic) {
			...
			while (true) {
				ch2 = *childs;
				childs++;
				auto _aic = (Verilog2001Parser::Attribute_instanceContext *)(ch2);
				if (_aic) {
			...
			childs++; // consume ":"
			auto ifTrue = visitExpression((Verilog2001Parser::ExpressionContext *)(ch2));
			assert(ifTrue);
			auto ifFalse = visitTerm((Verilog2001Parser::TermContext*)(*childs));
			assert(ifFalse);
...
// ;
	...
	for (auto ch : ctx->children) {
		if (!top)
			...
			Expr * second;
			auto ehbc = (Verilog2001Parser::Escaped_hierarchical_branchContext *)(ch);
			if (ehbc) {
				second = visitEscaped_hierarchical_branch(ehbc);
			} else {
				auto shbc = (Verilog2001Parser::Simple_hierarchical_branchContext *)(ch);
				assert(shbc);
